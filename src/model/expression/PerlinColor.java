...
public class PerlinColor extends Operation{
    private OperationProperties myProperties = new OperationProperties(new String[]{"perlinColor"}, 2);
    ...
    public PerlinColor (String command, List<Expression> operands) {
        myCommand = command;
        myOperands = operands;
    }
    ...
    public static OperationFactory getFactory() {
        return new OperationFactory(new PerlinColor());
    }

    @Override
    public Operation newOperation(String command, List<Expression> operands) {
        return new PerlinColor(command, operands);
    }

    @Override
    public RGBColor evaluateWithVariable(double evalX, double evalY, double currentTime, Map<String, RGBColor> varMap) {
        return ColorCombinations.perlinColor(myOperands.get(0).evaluateWithVariable(evalX, evalY, currentTime, varMap), myOperands.get(1).evaluateWithVariable(evalX, evalY, currentTime, varMap));
    }
