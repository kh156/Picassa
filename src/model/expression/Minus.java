...
private OperationProperties myProperties = new OperationProperties(new String[]{"minus", "-"}, 2);
    ...
    public Minus (String command, List<Expression> operands) {
        myCommand = command;
        myOperands = operands;
    }
    ...
    public static OperationFactory getFactory() {
        return new OperationFactory(new Minus());
    }

    @Override
    public Operation newOperation(String command, List<Expression> operands) {
        return new Minus(command, operands);
    }

    @Override
    public RGBColor evaluateWithVariable(double evalX, double evalY, double currentTime, Map<String, RGBColor> varMap) {
        return ColorCombinations.subtract(myOperands.get(0).evaluateWithVariable(evalX, evalY, currentTime, varMap), myOperands.get(1).evaluateWithVariable(evalX, evalY, currentTime, varMap));
    }
}
