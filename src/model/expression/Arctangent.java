...
public class Arctangent extends Operation{
    private OperationProperties myProperties = new OperationProperties(new String[]{"atan"}, 1);
    ...
    public Arctangent (String command, List<Expression> operands) {
        myCommand = command;
        myOperands = operands;
    }
    ...
    public static OperationFactory getFactory() {
        return new OperationFactory(new Arctangent());
    }

    @Override
    public Operation newOperation(String command, List<Expression> operands) {
        return new Arctangent(command, operands);
    }

    @Override
    public RGBColor evaluateWithVariable(double evalX, double evalY, double currentTime, Map<String, RGBColor> varMap) {
        return ColorCombinations.atan(myOperands.get(0).evaluateWithVariable(evalX, evalY, currentTime, varMap));
    }
