...
private OperationProperties myProperties = new OperationProperties(new String[]{"div", "/"}, 2);
    ...
    public Divide (String command, List<Expression> operands) {
        myCommand = command;
        myOperands = operands;
    }
    ...
    public static OperationFactory getFactory() {
        return new OperationFactory(new Divide());
    }

    @Override
    public Operation newOperation(String command, List<Expression> operands) {
        return new Divide(command, operands);
    }

    @Override
    public RGBColor evaluateWithVariable(double evalX, double evalY, double currentTime, Map<String, RGBColor> varMap) {
        return ColorCombinations.divide(myOperands.get(0).evaluateWithVariable(evalX, evalY, currentTime, varMap), myOperands.get(1).evaluateWithVariable(evalX, evalY, currentTime, varMap));
    }}